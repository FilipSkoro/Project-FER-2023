...
def get_Median_Distance(distance):
    ...
    if (len(Rpi1_dist) < 5):
        for key in distance.keys():
            if (key == "DC:A6:32:EA:37:A7"):    # Rpi1
                Rpi1_dist.append(distance.get(key))
    elif(len(Rpi1_dist) == 5):
        for key in distance.keys():
            if (key == "DC:A6:32:EA:37:A7"):
                Rpi1_dist.append(Rpi1_dist.pop(0))
                Rpi1_dist.remove(Rpi1_dist[4])
                Rpi1_dist.append(distance.get(key))
    ...
    if (Rpi0_dist):
        ...
        median_dis.append(0)
    if (Rpi1_dist):
        median_dis.append(statistics.median(Rpi1_dist))
    else:
        median_dis.append(0)
    if (Rpi2_dist):
...
def publish_Distances(median_dis):
    ...
    pub = rospy.Publisher('rpi3_topic', DistanceMsg, queue_size=10)
...
def callback(data):
    ...
    Rpi2_published_dist = list(data.list)    
...
def get_Matrix(median_dis):
    ...
    #dist_matrix[1] = Rpi1_published_dist
    #dist_matrix[2] = Rpi2_published_dist
    dist_matrix[3] = median_dis

    return dist_matrix

if __name__ == "__main__":

    mac_addresses = ["DC:A6:32:FB:26:63","DC:A6:32:EA:37:A7","DC:A6:32:D4:3F:56"]

    rospy.init_node('Rpi3', anonymous=True)
